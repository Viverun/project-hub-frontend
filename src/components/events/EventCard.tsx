...
export function EventCard({ event }: EventCardProps) {
    ...
    const handleInterest = async () => {
        setIsLoading(true);
        try {
            await eventApi.markInterest(event.id);
    ...
    return (
        ...
        >
            <Card className="flex flex-col h-full overflow-hidden transition-shadow hover:shadow-xl border-slate-200/60 bg-white/50 backdrop-blur-sm">
                <div className="aspect-video w-full bg-indigo-50 relative overflow-hidden flex items-center justify-center text-indigo-300">
                    {event.thumbnailUrl ? (
